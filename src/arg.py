...
def argz(argv=None, description=None, prevArgs=None):
    ...
    parser.add_argument("-C", "--coll",
                        default=str(
                            argDeflt(config, options, "coll", "testColl")),
    ...
    parser.add_argument("-P", "--port",
                        default=str(
                            argDeflt(config, options, "port", str("27017"))),
    ...
    parser.add_argument("--type",
                        default=str(
                            argDeflt(config, options, "type", str("lstm"))),
                        help="what loss function should be used")
    parser.add_argument("--timeSteps",
                        default=int(
                            argDeflt(config, options, "timeSteps", int(25))),
                        type=int,
                        help="how many unfolded rnn cells exist and inputs to expect to take")
    parser.add_argument("--testSize",
                        default=float(argDeflt(config, options,
                                               "testSize", float(0.2))),
                        type=float,
    ...
    parser.add_argument("--dimensionality",  # is this even neccessary?
                        default=int(argDeflt(config, options,
                                             "dimensionality", int(19))),
                        type=int,
                        help="total feature space")
    parser.add_argument("--layers",
                        default=int(
                            argDeflt(config, options, "layers", int(2))),
                        type=int,
    ...
    parser.add_argument("--epochs",
                        default=int(
                            argDeflt(config, options, "epochs", int(20))),
                        type=int,
    ...
    parser.add_argument("--toJustImport",
                        default=bool(argDeflt(config, options,
                                              "toJustImport", False, isBool=True)),
                        action="store_true",
                        help="sets flag to just import without cleaning")
    parser.add_argument("--pipeline",
                        default=str(argDeflt(config, options, "pipeline", str(
                            rootPath + "/config/pipeline.json"))),
                        help="set the path to the json pipeline file")
    parser.add_argument("--config",
                        default=str(argDeflt(config, options, "config", str(
    ...
    parser.add_argument("--customScript",
                        default=str(argDeflt(config, options, "customScript", str(
                            rootPath + "/examples/gan.py"))),
